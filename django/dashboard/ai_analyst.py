...
class MarineAIAnalyst:
    ...
    def analyze_clients(df):
        ...
        df = df.copy()
        # Ensure numeric
        # In Django usually we deal with list of dicts or ValuesQuerySet
        # Need to handle pandas DataFrame conversion if using it
        
        # ... assuming df is pandas DataFrame for now
    ...
    def analyze_correlations(corr_matrix):
        """
        Analyzes the correlation matrix (Pandas DataFrame) to find significant relationships.
        """
        insights = []
        ...
        if pairs:
            ...
            for p in top_pairs:
                ...
                if "Pendapatan" in [c1, c2]:
                    if val > 0: explanation = " Artinya: Jika satu naik, kemungkinan besar pendapatan juga ikut naik."
                    else: explanation = " Artinya: Hati-hati, jika faktor ini naik, pendapatan justru bisa turun."
                elif "Hari Bayar" in [c1, c2]:
                    if val > 0: explanation = " Ini bisa membuat uang masuk menjadi lebih lambat."
                    else: explanation = " Ini bagus karena bisa mempercepat penerimaan uang tunai."
                else:
                    if val > 0: explanation = " Keduanya cenderung naik atau turun bersamaan."
                    else: explanation = " Jika satu naik, yang lain biasanya turun."
