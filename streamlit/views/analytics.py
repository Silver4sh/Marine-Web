...
views/analytics.py
==================
UI layer untuk halaman Analitik.

Tanggung jawab:
  - Merender chart forecast, korelasi, aktivitas armada, logistik
  - TIDAK ada logika data fetching inline di body fungsi render
  - Semua import dari core dilakukan di top level
"""
...
from core import (
    ...
    get_fleet_daily_activity,
    calculate_advanced_forecast, apply_chart_style, calculate_correlation,
)
from core.ai_analyst import MarineAIAnalyst

# Streamlit fragment support (Streamlit ≥ 1.33; fallback untuk versi lama)
try:
    from streamlit import fragment
except ImportError:
    def fragment(func):   # type: ignore[misc]
        return func
...
def _render_ai_top_insight(fin_metrics: dict) -> None:
    """Merender satu insight AI keuangan di atas halaman."""
    ai_fin = MarineAIAnalyst.analyze_financials(fin_metrics)
    if not ai_fin['insights']:
        return
    ...
    st.markdown(
        ...
        <div style="background:linear-gradient(90deg,rgba(14,165,233,0.1) 0%,rgba(15,23,42,0) 100%);
            ...
            <div style="display:flex; gap:12px; align-items:start;">
                ...
                <div>
                    <div style="font-weight:600; color:#e0f2fe; margin-bottom:4px;">
                        {insight['title']}
                    </div>
...
def _render_correlation_insight(corr_matrix: pd.DataFrame) -> None:
    """Merender kartu insight AI kausalitas di samping heatmap korelasi."""
    st.markdown("##### 🤖 Analisis Kausalitas")
    analysis = MarineAIAnalyst.analyze_correlations(corr_matrix)
    for insight in analysis['insights']:
        color = "#ef4444" if insight['type'] == 'critical' else "#10b981"
        st.markdown(
...
@fragment
def render_overview_strip() -> None:
    """Strip metrik eksekutif — 4 KPI utama."""
...
@fragment
def render_revenue_forecast() -> None:
    """Chart proyeksi pendapatan 6 bulan (AI-powered)."""
    ...
    # 1. Confidence interval band
    x_band = pd.concat([forecast_df['month'], forecast_df['month'][::-1]])
    y_band = pd.concat([forecast_df['upper_bound'], forecast_df['lower_bound'][::-1]])
    fig.add_trace(go.Scatter(
        x=x_band, y=y_band, fill='toself',
        fillcolor='rgba(56,189,248,0.1)',
        line=dict(color='rgba(255,255,255,0)'),
        name='Interval Keyakinan 95%', hoverinfo="skip",
    ))

    # 2. Actual line
    actual = combined[combined['type'] == 'Aktual']
    fig.add_trace(go.Scatter(
    ...
    # 3. Forecast line
    forecast = combined[combined['type'] == 'Prakiraan']
    fig.add_trace(go.Scatter(
        x=forecast['month'], y=forecast['revenue'],
        mode='lines+markers', name='Proyeksi AI',
...
def render_correlation_section() -> None:
    """Heatmap korelasi Pearson + AI kausalitas."""
    ...
    with c1:
        ...
        st.plotly_chart(fig, width='stretch')
    with c2:
        _render_correlation_insight(corr_matrix)


@fragment
def render_fleet_activity_chart() -> None:
    """Heatmap intensitas operasional armada per hari."""
...
def render_analytics_page() -> None:
    """Entry point halaman Analitik — dipanggil dari main.py."""
    st.markdown("## 📈 Pusat Analitik")

    # AI insight banner (data diambil sekali, tidak dobel)
    fin_metrics = get_financial_metrics()
    _render_ai_top_insight(fin_metrics)

    # KPI strip
    render_overview_strip()
    st.markdown("---")

    # Main grid: 2/3 kiri — 1/3 kanan
