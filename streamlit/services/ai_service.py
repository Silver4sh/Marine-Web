...
class MarineSLM:
    ...
    def __init__(self):
        ...
        self.vocab = {
            ...
            "anomaly_terms": [
                "kapal hantu", "pergerakan tidak sah", "kelainan operasional", "penyimpangan kecepatan"
            ]
        }
    ...
    def generate(self, intent, context={}):
        ...
        # --- EXISTING INTENTS (Base v3.0) ---
            ...
            conn = random.choice(self.vocab["connectors"])
            result = random.choice(templates).format(
                opener=opener, count=context.get('count', 'beberapa'), 
                adj=adj, term=term, conn=conn
            )

        elif intent == "kpi_achievement":
            pct = context.get('pct', 0)
            templates_good = [
                "Pencapaian KPI bulan ini sangat {adj}: {pct}% dari target terpenuhi.",
                "{opener} kinerja {pct}% sudah melampaui ekspektasi. Pertahankan!",
                "Target tercapai {pct}%. Sistem berjalan dalam kondisi {adj}."
            ]
            templates_bad  = [
                "{opener} KPI bulan ini baru {pct}%. Diperlukan akselerasi segera.",
                "Pencapaian {pct}% terhadap target, {adj}. Cek bottleneck proses.",
                "Hanya {pct}% target terkumpul. Ada risiko {adj} di akhir periode."
            ]
            tpls   = templates_good if pct >= 80 else templates_bad
            opener = random.choice(self.vocab["openers_positive" if pct >= 80 else "openers_negative"])
            adj    = random.choice(self.vocab["adjectives_strong" if pct >= 80 else "adjectives_worrying"])
            result = random.choice(tpls).format(pct=pct, opener=opener, adj=adj)

        elif intent == "target_progress":
            pct = context.get('pct', 0)
            remaining = 100 - pct
            templates = [
                "Progress target: {pct}% tercapai, masih kekurangan {rem}% lagi untuk bulan ini.",
                "{opener} {pct}% target bulanan sudah terpenuhi. Sisa {rem}% harus dikejar.",
                "Realisasi {pct}% — {rem}% lagi menuju target. {adj}!"
            ]
            opener = random.choice(self.vocab["openers_positive" if pct >= 60 else "openers_negative"])
            adj = "Terus semangat" if pct >= 60 else "Perlu percepatan"
            result = random.choice(templates).format(
                pct=round(pct, 1), rem=round(remaining, 1), opener=opener, adj=adj
            )
...
class MarineAIAnalyst:
    ...
    def analyze_anomalies(anomaly_df):
        """Generate AI insight about vessel operational anomalies."""
        if anomaly_df is None or anomaly_df.empty:
            return {"insights": [{"title": "✅ Armada Normal", "desc": "Tidak ada anomali kapal yang terdeteksi dalam 2 jam terakhir.", "type": "positive"}]}
        count = len(anomaly_df)
        txt   = MarineAIAnalyst.slm.generate("vessel_anomaly", {"count": count})
        return {"insights": [{"title": f"🚨 {count} Anomali Kapal Aktif", "desc": txt, "type": "critical"}]}

    @staticmethod
    def analyze_kpi(achieved_pct: float):
        """Generate insight about KPI target achievement percentage."""
    ...
    def analyze_target_progress(progress_pct: float):
        """Narrate the progress towards the monthly revenue target."""
